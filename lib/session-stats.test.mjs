...
test("lastUsage follows the most recent request in file order", () => {
  const entries = [
    userEntry("u1", null),
    assistantEntry("a1", "u1", [{ type: "text", text: "old" }], usage({ input: 40 })),
    assistantEntry("a2", "a1", [{ type: "text", text: "new" }], usage({ input: 5 })),
  ];
  const stats = computeSessionStats(entries);
  assert.equal(stats.lastUsage.input, 5);
});
