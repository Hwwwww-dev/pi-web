...
test("names a running session from what it holds right now, without waiting for idle", async () => {
  ...
  // normalizeContext (pi-ai 0.87) folds the system prompt into a leading system message.
...
test("title request is a standalone stream and does not reuse the session prefix", () => {
  ...
  const head = request.context.messages[0];
  assert.equal(head?.role, "system");
  assert.equal(
    typeof head.content === "string" ? head.content : head.content.map((part) => part.text).join(""),
    "You name chat sessions from a transcript. Reply with the title only.",
  );
  assert.deepEqual(request.context.messages.slice(1).map((message) => message.role), ["user"]);
