...
test("prompt preflight accepts without mutating the system prompt", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  // The exact system prompt is projected per request by the
  // createExactSystemPromptExtension before_agent_start handler, so preflight
  // only admits the run and never rewrites the prompt state.
  assert.match(promptSource, /preflightResult: \(success\) => \{[\s\S]*?acceptPreflight\(\)/);
  assert.doesNotMatch(promptSource, /applyExactSystemPrompt/);
...
test("reloading a session invalidates the models cache", async () => {
  ...
  assert.doesNotMatch(reloadSource, /applyExactSystemPrompt/);
});
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /createExactSystemPromptExtension\(\(\) =>\n[\s\S]*?contextFilesSystemPrompt\(inner\.resourceLoader\.getAgentsFiles\(\)\.agentsFiles\)/);
