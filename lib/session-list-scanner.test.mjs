...
function fixture(t) {
  ...
  globalThis.__piWebScanIndexPersistDelayMs = 0;
  resetSessionScanIndexForTests();
  invalidateSessionListCache();
  t.after(() => {
    globalThis.__piWebScanIndexPersistDelayMs = undefined;
    resetSessionScanIndexForTests();
    invalidateSessionListCache();
...
// Persists are debounced off the request path now; poll until the latest
// in-memory state has actually landed on disk.
async function waitForIndexPersist(indexPath, predicate) {
  ...
  for (;;) {
    ...
    if (Date.now() > deadline) assert.fail("session index persist did not land in time");
...
test("matches SDK metadata and tie ordering across cache hits, mutations, and restarts", async (t) => {
  ...
  await check("warm", []);
  // Resets below drop the in-memory index, so the debounced persist must have
  // landed with the expected keys before each of them.
  const afterWarm = await waitForIndexPersist(
    indexPath,
    (parsed) => JSON.stringify(Object.keys(parsed.entries).sort()) === JSON.stringify([a, b, c].sort()),
  );
  ...
  await waitForIndexPersist(
    indexPath,
    (parsed) => JSON.stringify(Object.keys(parsed.entries).sort()) === JSON.stringify([a, b, d].sort()),
  );
  resetSessionScanIndexForTests();
  ...
  const expectedKeys = [a, d].sort();
  const persisted = await waitForIndexPersist(
    indexPath,
    (parsed) => JSON.stringify(Object.keys(parsed.entries).sort()) === JSON.stringify(expectedKeys),
  );
  assert.deepEqual(Object.keys(persisted.entries).sort(), expectedKeys);
});
...
test("discards malformed persisted entries and rebuilds the list from valid session files", async (t) => {
  ...
  const b = write("b", [message("b1", "user", "also healthy")]);
  const expected = await listSessionsIncremental();
  const pristine = JSON.stringify(await waitForIndexPersist(
    indexPath,
    (parsed) => JSON.stringify(Object.keys(parsed.entries).sort()) === JSON.stringify([a, b].sort()),
  ));
  const corruptions = [
...
test("a changed file is scanned in the same listing that notices the change", async (t) => {
  const { write } = fixture(t);
  const sessionPath = write("fresh", [
    message("f1", "user", "fresh request"),
    message("f2", "assistant", "fresh answer"),
  ]);

  // The listing is the only source for row counts, so a file whose stat moved
  // must come back fully scanned here. Deferring it to a later pass is what
  // printed "…" for the count of whatever session was being written.
  const [listed] = await listSessionsIncremental();
  assert.equal(listed.path, sessionPath);
  assert.equal(listed.messageCount, 2);
  assert.equal(listed.firstMessage, "fresh request");
  assert.equal(listed.id, "fresh");
  assert.equal(listed.cwd, process.env.PI_CODING_AGENT_DIR);
});
