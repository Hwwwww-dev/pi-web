...
test("commit summaries expose hash/shortHash/author/timestamp/subject", async (t) => {
  ...
  for (const commit of commits) {
    ...
    assert.equal(typeof commit.pushed, "boolean");
  }
});

test("pushed reports whether each commit is reachable from a remote branch", async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "pi-web-git-history-pushed-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const remote = await mkdtemp(path.join(os.tmpdir(), "pi-web-git-history-pushed-remote-"));
  t.after(() => rm(remote, { recursive: true, force: true }));
  await execFileAsync("git", ["init", "-q", "--bare", "-b", "main", remote]);
  await execFileAsync("git", ["init", "-q", "-b", "main", root]);
  await writeFile(path.join(root, "a.txt"), "one\n");
  await execFileAsync("git", ["-C", root, "add", "a.txt"]);
  await execFileAsync("git", ["-C", root, "commit", "-q", "-m", "pushed one"]);
  await execFileAsync("git", ["-C", root, "remote", "add", "origin", remote]);
  await execFileAsync("git", ["-C", root, "push", "-q", "origin", "main"]);
  await writeFile(path.join(root, "a.txt"), "one\ntwo\n");
  await execFileAsync("git", ["-C", root, "add", "a.txt"]);
  await execFileAsync("git", ["-C", root, "commit", "-q", "-m", "local only"]);
  const { commits } = await readCommitLog(root, DEFAULT_LOG_LIMIT, 0);
  assert.deepEqual(commits.map((commit) => [commit.subject, commit.pushed]), [
    ["local only", false],
    ["pushed one", true],
  ]);
});

test("readCommitDetail returns per-file and total line counts", async (t) => {
  const repo = await makeRepo(t);
  // HEAD (delete a) removes a.txt (2 lines); the rename commit R's b.txt unchanged.
  const { commits } = await readCommitLog(repo, DEFAULT_LOG_LIMIT, 0);
  const [head, rename, edit, add] = commits;
  const headDetail = await readCommitDetail(repo, head.hash);
  assert.deepEqual(headDetail.files, [
    { path: "a.txt", status: "D", additions: 0, deletions: 2 },
  ]);
  assert.equal(headDetail.totalAdditions, 0);
  assert.equal(headDetail.totalDeletions, 2);
  const editDetail = await readCommitDetail(repo, edit.hash);
  assert.deepEqual(editDetail.files, [
    { path: "a.txt", status: "M", additions: 1, deletions: 0 },
    { path: "b.txt", status: "A", additions: 1, deletions: 0 },
  ]);
  assert.equal(editDetail.totalAdditions, 2);
  assert.equal(editDetail.totalDeletions, 0);
  const renameDetail = await readCommitDetail(repo, rename.hash);
  assert.deepEqual(renameDetail.files, [
    { path: "c.txt", status: "R", previousPath: "b.txt", additions: 0, deletions: 0 },
  ]);
  const addDetail = await readCommitDetail(repo, add.hash);
  assert.deepEqual(addDetail.files, [
    { path: "a.txt", status: "A", additions: 1, deletions: 0 },
  ]);
});

test("readCommitDetail reports binary files with null counts", async (t) => {
  const repo = await makeRepo(t);
  await writeFile(path.join(repo, "img.bin"), Buffer.from([0, 1, 2, 0]));
  await execFileAsync("git", ["-C", repo, "add", "img.bin"]);
  await execFileAsync("git", ["-C", repo, "commit", "-q", "-m", "add binary"]);
  const { commits } = await readCommitLog(repo, 1, 0);
  const detail = await readCommitDetail(repo, commits[0].hash);
  assert.deepEqual(detail.files, [
    { path: "img.bin", status: "A", additions: null, deletions: null },
  ]);
  assert.equal(detail.totalAdditions, 0);
...
test("commit file bytes follow the revision and report a missing path as missing", async (t) => {
  const repo = await makeRepo(t);
  const { commits } = await readCommitLog(repo, DEFAULT_LOG_LIMIT, 0);
  const [head, , edit] = commits; // head deletes a.txt, edit still has it

  assert.equal((await readCommitFileBytes(repo, edit.hash, "a.txt")).toString("utf8"), "one\ntwo\n");
  await assert.rejects(
    () => readCommitFileBytes(repo, head.hash, "a.txt"),
    (error) => error instanceof CommitFileMissingError,
  );
  // A path that never existed and a bogus revision must not look the same.
  await assert.rejects(() => readCommitFileBytes(repo, edit.hash, "never-touched.txt"), CommitFileMissingError);
  await assert.rejects(() => readCommitFileBytes(repo, "0".repeat(40), "a.txt"), CommitFileMissingError);
});

test("commit file decoding rejects binary and truncates on a line boundary", () => {
  assert.deepEqual(decodeCommitFileText(Buffer.from("text\n")), { content: "text\n", truncated: false });
  assert.equal(decodeCommitFileText(Buffer.from([0x61, 0x00, 0x62])), null);

  const long = `${"x".repeat(99)}\n`.repeat(4_000);
  const truncated = decodeCommitFileText(Buffer.from(long));
  assert.equal(truncated.truncated, true);
  assert.ok(truncated.content.length <= TEXT_PREVIEW_MAX_BYTES);
  assert.ok(long.startsWith(truncated.content));
