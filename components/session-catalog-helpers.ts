 ...
 * plain spread can never clear one that stopped being true: a session that left
 * a worktree drops `branch`/`isWorktree`, and a fork whose parent went away
 * drops `relation`. Reset those from the refreshed row explicitly. Locally
 * applied fields such as an auto-generated title still survive until the next
