...
test("offers compact quoting controls and sends branch questions through the main chat", async () => {
  ...
  assert.match(shellSource, /initialPrompt=\{pendingQuotePrompt\?\.sessionId === activeChatSession\?\.id/);
  // One active instance plus the background keep-alive slot instances.
  assert.ok((shellSource.match(/<ChatWindow\b/g) ?? []).length >= 1);
